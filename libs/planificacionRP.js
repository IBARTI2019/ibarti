...
function rp_planif_trab_serv_create_cabecera(id_contenedor, callback, formato) {
  ...
  if (formato == "pdf") {
    divs
      ...
      .html(
        (d) =>
          ...
          d.values[0].values[0].values[0].cliente +
          " - " +
          d.values[0].values[0].values[0].ubicacion +
          ')" width="25px" src="imagenes/pdf.gif" border="0" onclick="rp_planif_serv_rp(\'pdf\',\'table' +
          d.key +
          '\')"> <select name="update" style="width:90px;" onchange="rp_planif_serv_update(' +
          d.key +
          ',this.value)">' +
    ...
    divs
      .append("span")
      .style("float", "right")
      .html((d) => "<b>DOBLE CLICK PARA MODIFICAR</b>");
  }

  var tablas = divs
    .append("table")
    .attr("id", (d) => "table" + d.key)
    .attr("width", "100%")
    .attr("class", "tabla_planif")
    .attr("align", "center");
  ...
  trh
    .append("th")
    .attr("colspan", (d) => 3 + Number(d.values.length))
    .attr("class", "etiqueta titulo")
    .text(
      (d) =>
        d.values[0].values[0].values[0].cliente +
        " - " +
  ...
  trh
    .append("th")
    .attr("colspan", (d) => d.values.length)
    .attr("class", "etiqueta")
    .text("Dias del Mes");
    ...
    if (map_res_mes_anio.has(d.key)) {
      map_res_mes_anio.get(d.key).values.forEach((d) => {
        trh
          .append("th")
          .attr("class", d.key)
          .attr("colspan", d.values.length)
          .on("mouseover", () =>
            d3.selectAll("." + d.key).classed("yellow", true)
          )
          .on("mouseout", () =>
            d3.selectAll("." + d.key).classed("yellow", false)
          )
          .text(d.key);
      });
    }
  ...
  res_fecha_cont.forEach((d) => {
    d3.select("#thead" + d.key)
      .append("tr")
      .selectAll("th")
      .data(d.values)
      .enter()
      .append("th")
      .attr("title", (d) => d.values[0].values[0].dia_semana)
      .attr(
        "class",
        (d) =>
          "etiqueta " +
          d.values[0].values[0].mes_anio +
          " " +
          d.values[0].values[0].mes_anio +
          d.key.substring(8)
      )
      .text((d) => d.values[0].values[0].dia_semana[0]);
  });
  res_fecha_cont.forEach((d) => {
    d3.select("#thead" + d.key)
      .append("tr")
  ...
  res_horario_contrato.forEach((ubic) => {
    ...
    ubic.values.forEach((horario) => {
      ...
      val_horarios.get(horario.key).values.forEach((cargo, i) => {
        ...
        trb = d3
          .select("#thead" + ubic.key)
          .append("tr")
          .attr(
            "id",
            (d) =>
              "tbody_contrato_" + ubic.key + "_" + horario.key + "_" + cargo.key
          )
          .attr("class", "color");
        if (i == 0) {
          trb
            .append("td")
            .attr("colspan", 2)
            .attr("rowspan", horario.values.length)
            .text((d) => horario.values[0].values[0].values[0].horario);
          trb
            .append("td")
            .attr("colspan", 1)
            .text((d) => cargo.values[0].values[0].cargo);
        } else {
          trb
            .append("td")
            .attr("colspan", 1)
          ...
          if (val_fechas.has(res)) {
            ...
            d3.select(
              ...
              .attr(
                "title",
                "CANTIDAD DEMANDA " +
                  horario.values[0].values[0].values[0].horario +
                  " " +
                  res
              )
              .attr(
                "class",
                "cantidad " +
                  val_fechas.get(res).values[0].mes_anio +
                  " " +
                  val_fechas.get(res).values[0].mes_anio +
                  res.substring(8)
              )
      ...
      trb = d3
        ...
        .attr(
          "id",
          (d) => "tbody_contrato_" + ubic.key + "_" + horario.key + "_total"
        )
        .attr("class", "color");
      trb
        .append("td")
        ...
        .text(
          (d) =>
            `TOTAL: ${
              val_horarios.get(horario.key).values[0].values[0].values[0]
                .horario
            }`
        );
      arreglo_dia.forEach((cantidad) => {
        trb
          ...
          .attr(
            "title",
            "CANTIDAD DEMANDA " + horario.values[0].values[0].horario
          )
          .attr("class", "cantidad ")
          .text(cantidad);
      });
...
function rp_planif_trab_serv_trabajador(tablas, callback) {
  ...
  var theads = tablas
    .append("thead")
    .attr("id", (d) => "thead_servicio_" + d.key);
  var tbodys = tablas
    .append("tbody")
    .attr("id", (d) => "tbody_servicio_" + d.key);
  ...
  res_fecha_cont.forEach((ubic, i) => {
    d3.select("#tr_servicio_fechas" + ubic.key)
      .selectAll(".fechas")
      .data(ubic.values)
      .enter()
      .append("th")
    ...
    if (val_ubic_f.has(ubic.key)) {
      val_ubic_f.get(ubic.key).values.forEach((d) => {
        ...
        trb = d3
          .select("#tbody_servicio_" + ubic.key)
          .append("tr")
          .attr("id", "tr_ficha_" + ubic.key + "_" + d.key);
        ...
        ubic.values.forEach((f) => {
          if (val_fechas.has(f.key)) {
            d3.select("#tr_ficha_" + ubic.key + "_" + d.key)
              .append("td")
              .attr("id", `cod_${val_fechas.get(f.key).values[0].cod_planif}`)
              .attr(
                "title",
                "(" +
                  d.key +
                  ") " +
                  d.values[0].values[0].ap_nombre +
                  " " +
                  f.key
              )
              .attr(
              ...
              .on("mouseover", (d) =>
                d3
                  .selectAll(
                    "." + f.values[0].values[0].mes_anio + f.key.substring(8)
                  )
                  .classed("yellow", true)
              )
              .on("mouseout", () =>
                ...
                if (conceptos) {
                  ...
                  if (
                    document.getElementById(
                      `select_${datos_map.get(f.key).values[0].cod_planif}`
                    ) != null
                  ) {
                    d3.select(
                      `#cod_${datos_map.get(f.key).values[0].cod_planif}`
                    ).text(`${datos_map.get(f.key).values[0].concepto}`);
                  } else {
                    let select = d3
                      .select(
                        `#cod_${datos_map.get(f.key).values[0].cod_planif}`
                      ...
                      .on("change", () => {
                        ...
                        guardar_cambio_concepto(
                          datos_map.get(f.key).values[0].cod_planif,
                          conceptos[index].turno,
                          () => {
                            B_reporte("F");
                          }
                        );
                    ...
                    conceptos.forEach((d) => {
                      if (datos_map.get(f.key).values[0].cod_turno == d.turno) {
                        select
                          ...
                          .text(`${d.descripcion}(${d.conceptos})`);
                      } else {
                        select
                          .append("option")
                          .attr("value", d.turno)
            ...
            d3.select("#tr_ficha_" + ubic.key + "_" + d.key)
              .append("td")
              .attr("id", `cod_${d.values[0].values[0].ficha}_${f.key}`)
              .attr(
                "title",
                "(" +
                  d.key +
              ...
              .on("dblclick", () => {
                if (conceptos) {
                  if (
                    document.getElementById(
                      `select_${d.values[0].values[0].ficha}_${f.key}`
                    ) != null
                  ) {
                    d3.select(
                      `#cod_${d.values[0].values[0].ficha}_${f.key}`
                    ).text("");
                  } else {
                    let parametros = {
                    ...
                    let select = d3
                      .select(`#cod_${d.values[0].values[0].ficha}_${f.key}`)
                      .text("")
                      .append("select")
                      .attr(
                        "id",
      ...
      d3.select("#tbody_servicio_" + ubic.key)
        .append("tr")
        .append("td")
        .text("Sin Planificacion de Trabajadores!..")
...
function rp_planif_trab_serv_conceptos(tablas, callback) {
  ...
  res_concepto.forEach((ubic, i) => {
    d3.select("#thead_conceptos_" + ubic.key)
      .selectAll("tr")
      .data(ubic.values)
      .enter()
      .append("tr")
      .attr("id", (d) => "tr_conceptos_" + ubic.key + "_" + d.key)
      .attr("class", "color");
    ubic.values.forEach((d, i) => {
      if (i == 0)
        d3.select("#tr_conceptos_" + ubic.key + "_" + d.key)
          .append("th")
          .attr("class", "etiqueta")
          .attr("colspan", 2)
          .attr("rowspan", ubic.values.length)
          .text((d) => "Resumen Conceptos");
      d3.select("#tr_conceptos_" + ubic.key + "_" + d.key)
        .append("td")
        .text(d.values[0].values[0].concepto);
      val_fechas = d3.map(d.values, (d) => d.key);
...
function rp_planif_trab_serv_horario(tablas, callback) {
  ...
  res_horario.forEach((ubic, i) => {
    d3.select("#thead_horarios_" + ubic.key)
      .selectAll("tr")
      .data(ubic.values)
      .enter()
      .append("tr")
      .attr("id", (d) => "tr_horarios_" + ubic.key + "_" + d.key)
      .attr("class", "color");
    ubic.values.forEach((d, i) => {
      if (i == 0)
        d3.select("#tr_horarios_" + ubic.key + "_" + d.key)
          .append("th")
          .attr("class", "etiqueta")
          .attr("colspan", 2)
          .attr("rowspan", ubic.values.length)
      ...
      d3.select("#tr_horarios_" + ubic.key + "_" + d.key)
        .append("td")
        .text(d.values[0].values[0].concepto_horario);
      val_fechas = d3.map(d.values, (d) => d.key);
      if (val_ubic.has(ubic.key)) {
        val_ubic.get(ubic.key).values.forEach((f) => {
          if (val_fechas.has(f.key)) {
            d3.select("#tr_horarios_" + ubic.key + "_" + d.key)
              ...
              .attr(
                "title",
                "RESUMEN HORARIO (" +
                  d.values[0].values[0].concepto_horario +
                  ") " +
                  f.key
              )
              .attr(
...
function rp_planif_trab_serv_diferencia(tablas, callback) {
  ...
  var theads = tablas
    .append("thead")
    .attr("id", (d) => "thead_diferencia_" + d.key);
  ...
  res_fecha_cont.forEach((d) => {
    d3.select("#tr_diferencia_" + d.key)
      .selectAll(".fechas")
      .data(d.values)
      .enter()
      .append("th")
  ...
  res_horario_cont.forEach((ubic, i) => {
    if (val_ubic_f.has(ubic.key)) {
      ...
      ubic.values.forEach((horario, i) => {
        val_fechas = d3.map(horario.values, (d) => d.key);
        trb = d3
          .select("#tbody_diferencia_" + ubic.key)
          .append("tr")
          .attr("id", (d) => "tr_diferencia_" + ubic.key + "_" + horario.key)
          .attr("class", "color");
        trb
          .append("td")
          .attr("colspan", 3)
          .text((d) => horario.values[0].values[0].horario);
        val_ubic.get(ubic.key).values.forEach((f) => {
          if (val_fechas.has(f.key)) {
            val_fechas.get(f.key).values.forEach((d) => {
              cantidad += Number(d.cantidad);
          ...
          var cantidad_horario = 0,
            diff = 0;
          if (val_ubic_h.has(ubic.key)) {
            val_horarios_h = d3.map(
              val_ubic_h.get(ubic.key).values,
              (d) => d.key
            );
            if (val_horarios_h.has(horario.key)) {
              val_fechas_h = d3.map(
                val_horarios_h.get(horario.key).values,
                (d) => d.key
              );
          ...
          d3.select("#tr_diferencia_" + ubic.key + "_" + horario.key)
            .append("td")
            .attr(
              "title",
              "DIFERENCIA " + horario.values[0].values[0].horario + " " + f.key
            )
            .on("mouseover", (d) =>
              d3
                .selectAll(
...
function rp_planif_serv_update(cod_ubicacion, tipo) {
  ...
  map_res_horario_cont.get(cod_ubicacion).values.forEach((horario) => {
    val_fechas = d3.map(horario.values, (d) => d.key);
    trb = d3
      .select("#tbody_contrato_" + cod_ubicacion + "_" + horario.key + "_total")
      .selectAll(".cantidad")
      .remove();
    trb
      .append("td")
    ...
    val_ubic.get(cod_ubicacion).values.forEach((f) => {
      ...
      d3.select(
        "#tbody_contrato_" + cod_ubicacion + "_" + horario.key + "_total"
      )
        .append("td")
        .attr(
          "title",
          "CANTIDAD DEMANDA " +
            horario.values[0].values[0].horario +
            " " +
            f.key
        )
        .attr(
  ...
  val_ubic_f.get(cod_ubicacion).values.forEach((d) => {
    val_fechas = d3.map(d.values, (d) => d.key);
    trb = d3
      .select("#tr_ficha_" + cod_ubicacion + "_" + d.key)
      .selectAll(".cantidad")
      .remove();
    map_res_fec_cont.get(cod_ubicacion).values.forEach((f) => {
      if (val_fechas.has(f.key)) {
        horas = 0;
        if (tipo == "H") {
          d3.select("#tr_ficha_" + cod_ubicacion + "_" + d.key)
            .append("td")
            .attr(
              "title",
              "(" + d.key + ") " + f.values[0].ap_nombre + " " + f.key
            )
            .attr(
              "class",
              "cantidad " +
            ...
            .on("mouseover", () =>
              d3
                .selectAll("." + f.values[0].mes_anio + f.key.substring(8))
                .classed("yellow", true)
            )
            .on("mouseout", () =>
              d3
          ...
          d3.select("#tr_ficha_" + cod_ubicacion + "_" + d.key)
            ...
            .attr(
              "title",
              "(" + d.key + ") " + d.values[0].values[0].ap_nombre + " " + f.key
            )
            .attr(
              "class",
              "cantidad " +
  ...
  map_res_horario.get(cod_ubicacion).values.forEach((d, i) => {
    d3.select("#tr_horarios_" + cod_ubicacion + "_" + d.key)
      .selectAll(".cantidad")
      .remove();
    val_fechas = d3.map(d.values, (d) => d.key);
    val_ubic.get(cod_ubicacion).values.forEach((f) => {
  ...
  map_res_horario_cont.get(cod_ubicacion).values.forEach((horario, i) => {
    val_fechas = d3.map(horario.values, (d) => d.key);
    d3.select("#tr_diferencia_" + cod_ubicacion + "_" + horario.key)
      .selectAll(".cantidad")
      .remove();
    val_ubic.get(cod_ubicacion).values.forEach((f) => {
      if (val_fechas.has(f.key)) {
      ...
      if (val_ubic_h.has(cod_ubicacion)) {
        val_horarios_h = d3.map(
          val_ubic_h.get(cod_ubicacion).values,
          (d) => d.key
        );
        if (val_horarios_h.has(horario.key)) {
          val_fechas_h = d3.map(
...
function rp_planif_trab_serv_create_detalle(id_contenedor, callback) {
  ...
  divs
    ...
    .html(
      (d) =>
        ...
        ')" width="25px" src="imagenes/excel.gif" border="0" onclick="rp_planif_serv_rp(\'excel\',\'table' +
        d.key +
        '\')"> <select name="region" style="width:90px;" onchange="rp_planif_serv_update_detalle(' +
        d.key +
        ',this.value)">' +
        '<option value="C"> CANTIDADES</option><option value="H">HORAS</option></select>'
    );
  ...
  trh
    .append("th")
    .attr("colspan", (d) => 4 + Number(d.values.length))
    .attr("class", "etiqueta titulo")
    .text(
      (d) =>
        d.values[0].values[0].values[0].values[0].cliente +
        " - " +
        d.values[0].values[0].values[0].values[0].ubicacion
    );
  ...
  res_fecha_cont.forEach((d, i) => {
    d3.select("#thead" + d.key)
      ...
      .enter()
      .append("th")
      .attr("title", (d) => d.values[0].values[0].values[0].dia_semana)
      .attr(
        "class",
        (d) =>
          "etiqueta " +
          d.values[0].values[0].values[0].mes_anio +
          " " +
          d.values[0].values[0].values[0].mes_anio +
          d.key.substring(8)
      )
      .text((d) => d.values[0].values[0].values[0].dia_semana[0]);
  });
  res_fecha_cont.forEach((d, i) => {
    d3.select("#thead" + d.key)
      .append("tr")
  ...
  res_horario_cont.forEach((ubic) => {
    ...
    ubic.values.forEach((puesto, i) => {
      ...
      d3.select(
        ...
        .append("td")
        .attr("rowspan", puesto.values.length)
        .attr("colspan", 2)
        .text(puesto.values[0].values[0].values[0].puesto);
      ...
      puesto.values.forEach((d) => {
        ...
        d3.select(
          ...
          .append("td")
          .attr("colspan", 2)
          .text(`TOTAL: ${d.values[0].values[0].horario}`);
        val_ubic.get(ubic.key).values.forEach((f) => {
          if (val_fechas.has(f.key)) {
          ...
          d3.select(
            ...
            .attr(
              "title",
              "CANTIDAD DEMANDA " +
                d.values[0].values[0].puesto +
                " (" +
                d.values[0].values[0].horario +
                ") " +
                f.key
            )
            .attr(
              "class",
              "cantidad " +
                f.values[0].values[0].values[0].mes_anio +
                " " +
                f.values[0].values[0].values[0].mes_anio +
                f.key.substring(8)
            )
  ...
  val_ubic_f = d3.map(res_ficha, (d) => d.key);
  res_fecha_cont.forEach((ubic, i) => {
    d3.select("#tr_servicio_fechas_" + ubic.key)
      .selectAll(".fechas")
      .data(ubic.values)
      .enter()
      .append("th")
    ...
    ubic.values.forEach((puesto, i) => {
      puesto.values.forEach((d) => {
        d3.select("#thead_conceptos_" + ubic.key)
          ...
          .attr(
            "id",
            "tr_conceptos_" + ubic.key + "_" + puesto.key + "_" + d.key
          )
          .attr("class", "color");
      });
      ...
      puesto.values.forEach((d) => {
        d3.select("#tr_conceptos_" + ubic.key + "_" + puesto.key + "_" + d.key)
          .append("td")
          .text(d.values[0].values[0].concepto);
        val_fechas = d3.map(d.values, (d) => d.key);
        if (val_ubic.has(ubic.key)) {
    ...
    ubic.values.forEach((puesto, i) => {
      puesto.values.forEach((d) => {
        d3.select("#thead_horarios_" + ubic.key)
          ...
          .attr(
            "id",
            "tr_horarios_" + ubic.key + "_" + puesto.key + "_" + d.key
          )
          .attr("class", "color");
      });
      ...
      puesto.values.forEach((d) => {
        d3.select("#tr_horarios_" + ubic.key + "_" + puesto.key + "_" + d.key)
          .append("td")
          .text(d.values[0].values[0].horario);
        val_fechas = d3.map(d.values, (d) => d.key);
        if (val_ubic.has(ubic.key)) {
          val_ubic.get(ubic.key).values.forEach((f) => {
            if (val_fechas.has(f.key)) {
  ...
  res_horario_cont.forEach((ubic) => {
    ...
    ubic.values.forEach((puesto, i) => {
      val_horarios = d3.map(puesto.values, (d) => d.key);
      puesto.values.forEach((d) => {
        d3.select("#tbody_diferencia_" + ubic.key)
          .append("tr")
          .attr(
            "id",
            "tr_diferencia_" + ubic.key + "_" + puesto.key + "_" + d.key
          )
          .attr("class", "color");
      });
      ...
      puesto.values.forEach((d) => {
        val_fechas = d3.map(d.values, (d) => d.key);
        d3.select("#tr_diferencia_" + ubic.key + "_" + puesto.key + "_" + d.key)
          .append("td")
          .attr("colspan", 2)
          .text(d.values[0].values[0].horario);
        val_ubic.get(ubic.key).values.forEach((f) => {
          ...
          var cantidad_horario = 0,
            diff = 0;
          if (val_ubic_h.has(ubic.key)) {
            val_puestos_h = d3.map(
              val_ubic_h.get(ubic.key).values,
              (d) => d.key
            );
            if (val_puestos_h.has(puesto.key)) {
              val_horarios_h = d3.map(
                val_puestos_h.get(puesto.key).values,
                (d) => d.key
              );
              if (val_horarios_h.has(d.key)) {
                val_fechas_h = d3.map(
                  val_horarios_h.get(d.key).values,
                  (d) => d.key
                );
                if (val_fechas_h.has(f.key)) {
                  cantidad_horario += val_fechas_h.get(f.key).values.length;
...
function rp_planif_serv_update_detalle(cod_ubicacion, tipo) {
  ...
  map_res_horario.get(cod_ubicacion).values.forEach((puesto, i) => {
    puesto.values.forEach((d) => {
      d3.select(
        "#tr_horarios_" + cod_ubicacion + "_" + puesto.key + "_" + d.key
      )
        .selectAll(".cantidad")
        .remove();
    });
      ...
      val_ubic.get(cod_ubicacion).values.forEach((f) => {
        ...
        if (val_fechas.has(f.key)) {
          ...
          d3.select(
            ...
            .attr(
              "class",
              "cantidad " + f.values[0].values[0].mes_anio + f.key.substring(8)
            )
            .text(0);
        }
      });
  ...
  map_res_horario_cont.get(cod_ubicacion).values.forEach((puesto, i) => {
    ...
    puesto.values.forEach((d) => {
      d3.select(
        "#tr_diferencia_" + cod_ubicacion + "_" + puesto.key + "_" + d.key
      )
        .selectAll(".cantidad")
        .remove();
    });
  ...
  if (d3.select("#" + id_contenedor).node()) {
    ...
    res_contratacion.forEach((d) => {
      d3.select("#body_" + d.key)
        ...
        .attr("class", (e) => {
          ...
          if (data["excepcion"] !== undefined) {
            if (map_res_excepcion.has(e.key)) {
              excepcion = Number(
                map_res_excepcion.get(e.key).values[0].cantidad
              );
            }
          }

          let valorMax = e.values.reduce((previous, current) => {
            return Number(current.trab_neces) > Number(previous.trab_neces)
              ? current
              : previous;
          });
          e.values = [valorMax];
        ...
        .html((e) => {
          (factor = 0), (trab_neces = 0), (trab_activos = 0), (excepcion = 0);
          cantidad = 0;
          ...
          return (
            '<td class="texto" id="center" >' +
            e.values[0].region +
            '</td><td class="texto" id="center" >' +
            e.values[0].estado +
            '</td><td class="texto" id="center" >' +
            e.values[0].cod_cliente +
            '</td><td class="texto" id="center" >' +
            e.values[0].cliente +
            '</td><td class="texto" id="center" >' +
            e.values[0].cod_ubicacion +
            '</td><td class="texto" id="center" >' +
            e.values[0].ubicacion +
            '</td><td class="texto" id="center" >' +
            cantidad +
            '</td><td class="texto" id="center" >' +
            trab_neces +
...
function rp_planif_serv_vs_contratacion_horario(data, id_contenedor, callback) {
  if (d3.select("#" + id_contenedor).node()) {
    ...
    d3.select("#thead")
      ...
      .attr("class", "fondo00")
      .html(
        '<th width="8%" class="etiqueta">Fecha</th><th width="20%" class="etiqueta">Horario</th><th width="20%" class="etiqueta">Estado</th><th width="22%" class="etiqueta">Cliente<th width="22%" class="etiqueta">Ubicacion<th width="8%" class="etiqueta">Factor</th>'
      );
    var horarios = [];
    var ubicaciones = [];
    var fechas = [];
    data["contrato"].forEach((a) => {
      ...
      if (map_res_horario.has(a.cod_ubicacion)) {
        val_ubic_h = d3.map(
          map_res_horario.get(a.cod_ubicacion).values,
          (d) => d.key
        );
        if (val_ubic_h.has(a.cod_horario)) {
          val_ubic_f = d3.map(
            val_ubic_h.get(a.cod_horario).values,
            (d) => d.key
          );
          if (val_ubic_f.has(a.fecha)) {
            val_ubic_f.get(a.fecha).values.forEach((d) => {
              sum_dia += Number(d.valor);
            });
          }
      ...
      res_horario.forEach((ubicacion) => {
        ubicacion.values.forEach((horario) => {
          horario.values.forEach((fecha) => {
            ...
            const existeEnContratos = data["contrato"].some(
              (contrato) =>
            ...
            if (!existeEnContratos) {
              ...
              $("#tbody_pl_vs_as").append(
                '<tr class="' +
                  ...
                  a.fecha +
                  '</td><td class="texto" id="center" >' +
                  val_ubic_f.get(a.fecha).values[0].horario +
                  '</td><td class="texto" id="center" >' +
                  a.estado +
                  '</td><td class="texto" id="center" >' +
                  a.cliente +
...
function rp_planif_trab_vs_asistencia_region(data, id_contenedor, callback) {
  if (d3.select("#" + id_contenedor).node()) {
    ...
    d3.select("#thead")
      ...
      .attr("class", "fondo00")
      .html(
        '<th width="8%" class="etiqueta">Fecha</th><th width="20%" class="etiqueta">Region</th><th width="20%" class="etiqueta">Horario</th><th width="8%" class="etiqueta">Factor</th>'
      );
    ...
    d3.select("#tbody_pl_vs_as")
      ...
      .attr("class", (a) => {
        ...
        if (map_res_horario.has(a.cod_region)) {
          val_ubic_h = d3.map(
            map_res_horario.get(a.cod_region).values,
            (d) => d.key
          );
          if (val_ubic_h.has(a.cod_horario)) {
            val_ubic_f = d3.map(
        ...
        if (map_res_horario.has(a.cod_region)) {
          ...
          if (val_ubic_h.has(a.cod_horario)) {
            ...
            return (
              ...
              '<td class="texto" id="center" >' +
              a.region +
              '</td></td><td class="texto" id="center" >' +
              a.horario +
              '</td><td class="texto" id="center">' +
              (0 - Number(a.cantidad)) +
              "</td>"
