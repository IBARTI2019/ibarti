...
function replicar_rot() {
  if (
    confirm("Esta seguro(a) de que desea replicar las rotaciones anteriores?..")
  ) {
    ubic = $("#planf_ubicacion").val();
    var parametros = {
      apertura: apertura,
...
function newPlanifIA() {
  ...
  if (apertura && ubic && contratacion) {
    if (
      confirm(
        "Esta seguro(a) de que desea planificar una nueva apertura mensual con IA?.."
      )
    ) {
      var parametros = {
        cod_apertura: apertura,
        cod_contratacion: contratacion,
        cod_cliente: cliente,
        cod_ubic: ubic,
        cod_usuario: usuario,
      };
      $.ajax({
        ...
        beforeSend: function () {
          $("#cont_planif_det").html(
            '<img src="imagenes/loading3.gif" border="null" class="imgLink" width="30px" height="30px"> Espere a que la IA analize los requerimientos, tenga paciencia, esto puede tardar varios minutos..'
          );
        },
        success: function (response) {
          $("#cont_planif_det").html(
            '<img src="imagenes/loading3.gif" border="null" class="imgLink" width="30px" height="30px"> Procesando respuesta de la IA..'
          );
      ...
      if (response.error) {
        ...
        alert(
          "Cobertura de requerimientos: " +
            response.meta.cobertura_requerimientos +
            "\n\nEstrategia: " +
            response.meta.estrategia +
            "\n\nRecursos utilizados: " +
            response.meta.recursos_utilizados
        );
      }
    },
    error: function (xhr, ajaxOptions, thrownError) {
...
function save_planif_apertura() {
  ...
  $.ajax({
    ...
    type: "post",
    beforeSend: function () {
      $("#planif_apertura_ing").html(
        '<img src="imagenes/loading3.gif" border="null" class="imgLink" width="30px" height="30px">'
      );
    },
    success: function (response) {
      $("#planif_apertura_ing").html(
        '<span class="art-button-wrapper" id="planif_apertura_ing"><span class="art-button-l"> </span><span class="art-button-r"> </span><input type="submit" name="salvar"  id="salvar" value="Guardar" class="readon art-button" /> </span'
      );
      var resp = JSON.parse(response);
      if (resp.error) {
        alert(resp.mensaje);
...
function save_planif_det(cod) {
  ...
  if (error == 0) {
    ...
    $.ajax({
      ...
      beforeSend: function () {
        $("#cont_planif_det").html(
          '<img src="imagenes/loading3.gif" border="null" class="imgLink" width="35px" height="35px"> Procesando...'
        );
      },
      success: function (response) {
        cargar_planif_det(ubic);
...
function Borrar_trab_det(cod) {
  ...
  var puesto = $("#det_puesto_trab" + cod + "").val();
  var usuario = $("#usuario").val();
  if (
    confirm(
      "�Esta Seguro De Borrar el Registro(" + ficha + ") de esta Planificación?"
    )
  ) {
    var parametros = {
      ficha: ficha,
  ...
  if (errorMessage == "") {
    $.ajax({
      ...
      beforeSend: function () {
        ...
        $("#modalRP").show();
        $("#RP").html(
          '<img src="imagenes/loading3.gif" border="null" class="imgLink" width="30px" height="30px"> Procesando...'
        );
      ...
      success: function (response) {
        ...
        if (typeof resp["servicio"] == "undefined") {
          ...
          if (detalle == "T") {
            rp_planif_trab_serv_detalle(resp, "modal_contenidoRP", () =>
              $("#body_planif").val($("#t_reporte").html())
            );
          } else if (detalle == "R") {
            rp_planif_trab_servresumida(resp, "modal_contenidoRP", () =>
              $("#body_planif").val($("#t_reporte").html())
            );
          }
          $("#RP").html("");
...
function mod_apertura_planif() {
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      var resp = JSON.parse(response);
      var tabla = d3
        .select("#modal_contenido")
        .append("div")
        .style("overflow", "scroll")
        .style("max-height", "500px")
        .append("table")
        .attr("class", "tabla_sistema")
        .attr("width", "100%");
      ...
      var tr = tbody
        ...
        .enter()
        .append("tr")
        .attr("id", (d) => "cod_" + d.codigo)
        .attr("class", "color")
        .attr(
          "title",
          (d) => "ULTIMA MODIFICACION: " + d.fecha_mod + " (" + d.nombres + ")"
        );
      ...
      elementos_cantidad
        ...
        .attr(
          "id",
          (d) =>
            `cant_${d.codigo}_${d.fecha}_${d.cod_cliente}_${d.cod_puesto}_${d.cod_ubicacion}_${d.cod_turno}_${d.cod_cargo}`
        )
        .attr("type", "number")
        .attr("min", "0")
        .attr("value", (d) => d.cantidad)
        .style("width", "60px")
        .on("click", (d, e, f) => {
          if (Number(f[e].value) == Number(f[e].defaultValue)) {
            $(
              `#mod_${d.codigo}_${d.fecha}_${d.cod_cliente}_${d.cod_puesto}_${d.cod_ubicacion}_${d.cod_turno}_${d.cod_cargo}`
            ).hide();
          } else {
            $(
              `#mod_${d.codigo}_${d.fecha}_${d.cod_cliente}_${d.cod_puesto}_${d.cod_ubicacion}_${d.cod_turno}_${d.cod_cargo}`
      ...
      elementos_cantidad
        ...
        .attr("src", "imagenes/ico_guadar.ico")
        .style("width", "20px")
        .style("margin-left", "10px")
        .style("display", "none")
        .attr("title", "modificar registro")
        .on("click", (d) => {
          if (confirm("Esta seguro que desea modificar")) {
            ...
            ac_apertura_planif(propiedades, () => {
              ...
              $(
                `#cant_${d.codigo}_${d.fecha}_${d.cod_cliente}_${d.cod_puesto}_${d.cod_ubicacion}_${d.cod_turno}_${d.cod_cargo}`
              )[0].defaultValue = propiedades.cantidad;
              cerrar = true;
            });
          }
        });
